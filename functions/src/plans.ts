...
// Use the module-level Firestore instance initialised in index.ts (singleton).
// Calling admin.firestore() here is safe — the Admin SDK always returns the
// same underlying instance after initializeApp() has been called.
...
export async function getAllPlans(): Promise<Map<string, PlanConfig>> {
  ...
  logger.info("plans Cold start — fetching plans from Firestore");
  await refreshCache();
  logger.info("plans Loaded", { count: _cache!.size, plans: [..._cache!.keys()] });
  return _cache!;
}
