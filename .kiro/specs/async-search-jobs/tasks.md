...
- [ ] 1. Add shared types and helper utilities
  - [ ] 1.1 Add job-related TypeScript types to `functions/src/types.ts`
  ...
  - [ ] 1.2 Create `functions/src/jobHelpers.ts` with deterministic job ID computation and subcollection cleanup
  ...
  - [ ]* 1.3 Write property tests for `computeJobId`
    - **Property 3: Deterministic job ID is a pure function**
    - **Validates: Requirements 7.1**

- [ ] 2. Implement Job_Creator Cloud Function
  - [ ] 2.1 Refactor `dataforseoBusinessSearch` in `functions/src/index.ts` into the Job_Creator
  ...
  - [ ]* 2.3 Write property test for duplicate running job returns existing ID
    - **Property 4: Duplicate running job returns existing ID**
    - **Validates: Requirements 7.3**
  - [ ]* 2.4 Write property test for terminal job reuse clears stale results
    - **Property 5: Terminal job reuse clears stale results**
    - **Validates: Requirements 7.4**

- [ ] 3. Implement Job_Processor Cloud Function
  - [ ] 3.1 Create the `processSearchJob` Firestore onCreate trigger in `functions/src/index.ts`
  ...
  - [ ] 3.2 Implement pipeline execution with progress writes and cancellation checks
  ...
  - [ ]* 3.3 Write property test for pipeline completion invariant
    - **Property 6: Pipeline completion invariant**
    - **Validates: Requirements 3.2, 3.3, 3.4**
  - [ ]* 3.4 Write property test for partial enrichment failure
...
- [ ] 5. Implement Job_Canceller Cloud Function
  - [ ] 5.1 Create the `cancelSearchJob` HTTP function in `functions/src/index.ts`
  ...
  - [ ]* 5.2 Write property test for cancel endpoint ownership check
    - **Property 11: Cancel endpoint ownership check**
    - **Validates: Requirements 4.1**

- [ ] 6. Implement cleanup scheduled functions
  - [ ] 6.1 Create `cleanupStuckJobs` scheduled function in `functions/src/index.ts`
    ...
    - _Requirements: 8.3_
  - [ ] 6.2 Create `cleanupExpiredJobs` scheduled function in `functions/src/index.ts`
    ...
    - _Requirements: 8.2_
  - [ ]* 6.3 Write property test for stuck job cleanup
    - **Property 17: Stuck job cleanup marks only old running jobs**
    - **Validates: Requirements 8.3**
  - [ ]* 6.4 Write property test for TTL cleanup
    - **Property 16: TTL cleanup deletes only expired jobs**
    - **Validates: Requirements 8.2**

- [ ] 7. Update Firestore security rules and Firebase config
  - [ ] 7.1 Add security rules for `jobs` collection and `results` subcollection in `firestore.rules`
  ...
  - [ ] 7.2 Add hosting rewrite for `/api/search/cancel` in `firebase.json`
    - Add rewrite entry before the existing `/api/search` entry
    - _Requirements: 4.5_

- [ ] 8. Checkpoint - Ensure all backend functions compile and tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Implement frontend `useSearchJob` hook
  - [ ] 9.1 Create `frontend/src/hooks/useSearchJob.ts`
  ...
  - [ ]* 9.2 Write property test for results sorted by score descending
    - **Property 14: Results sorted by score descending**
    - **Validates: Requirements 5.3**
  - [ ]* 9.3 Write property test for progress display state machine
    - **Property 15: Progress display state machine**
    - **Validates: Requirements 5.2**

- [ ] 10. Update frontend API client and search page
  - [ ] 10.1 Update `frontend/src/lib/api.ts`
    ...
    - _Requirements: 10.4_
  - [ ] 10.2 Update `frontend/src/pages/Index.tsx` to use `useSearchJob` hook
...
- [ ] 11. Checkpoint - Ensure frontend compiles and all tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Wire everything together and final verification
  - [ ] 12.1 Remove the old synchronous pipeline code from `functions/src/index.ts`
  ...
  - [ ]* 12.2 Write property test for normalization equivalence
